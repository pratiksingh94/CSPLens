...
export function SourceChip({ source, onRemove }: Props) {
  ...
  return (
    <span className={
      cn("inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-mono", getVariantClasses())
    }>
      {source}
      <button onClick={onRemove} className="hover:opacity-70 transition-opacity"><X className="h-3 w-3"/></button>
    </span>
  )
}