...
import { AlertCircle, AlertTriangle, Check, Copy, ExternalLink, Layers, Search, Shield, Sparkles, XCircle } from "lucide-react";
...
export function PolicyBuilder() {
  ...
  const filteredDirective = CSP_DIRECTIVES.filter(d => {
    ...
    return d.name.toLowerCase().includes(searchQuery.toLowerCase()) || d.description.toLowerCase().includes(searchQuery.toLowerCase());
  })
  const booleanDirectivesList = CSP_DIRECTIVES.filter(d => d.boolean === true);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        ...
        <Button variant="outline" size="sm" onClick={addSelfToAll} disabled={enabledCount === 0} className="cursor-pointer">+ &apos;self&apos;</Button>
        <Button variant="outline" size="sm" onClick={clearAll} disabled={enabledCount === 0} className="cursor-pointer">Clear</Button>
      </div>
        ...
        <div className="space-y-4 animate-in fade-in">
          ...
          <div className="rounded-lg border border-border bg-card p-4">
            <div className="flex items-center gap-4">
              <div className={cn("relative w-20 h-20 rounded-full flex items-center justify-center shrink-0", getGradeBg(overview.policyGrade.grade))}>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className={cn("text-2xl font-bold", getGradeColor(overview.policyGrade.grade))}>
                    {overview.policyGrade.grade}
                  </span>
                  <span className="text-xs text-muted-foreground">{overview.policyGrade.score}%</span>
                </div>
              </div>
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground">
                  {
                    overview.policyGrade.score >= 90 ? "Execellent! Your policy follows best security pratices." : overview.policyGrade.score >= 70 ? "Good, but there are improvements to consider." : overview.policyGrade.score >= 50 ? "Moderate risk. Review the issues below." : "High risk! Your policy needs significant improvements."
                  }
                </p>
              </div>
            </div>
          ...
          <div className="rounded-lg border border-border bg-card overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              <label className="flex items-center gap-1.5 text-xs text-muted-foreground cursor-pointer">
                ...
                <Switch checked={reportOnly} onCheckedChange={setReportOnly} className="scale-75"/>
              </label>
            ...
            <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4">
              ...
              <div className="space-y-3">
                {overview.redFlags.map(flag => (
                  <div key={flag.key} className="p-3 rounded-md bg-card border border-border">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {flag.recommendation && (<p className="text-sm text-primary mt-1">{flag.recommendation}</p>)}
                        {flag.references && (
                          <a
                        ...
                        <div className="flex flex-wrap gap-1 mt-2">
                          ...
                          {flag.directives.map(d => (
                            <code key={d} className="px-1.5 py-0.5 bg-muted rounded text-xs">
                              {d}
                            </code>
                          ))}
                        </div>
          ...
          {overview.missingDirectives.length > 0 && (
            <div className="rounded-lg border border-warning/30 bg-warning/5 p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-semibold text-sm text-warning">Missing Directives ({overview.missingDirectives.length})</span>
              </div>

              <div className="space-y-3">
                {overview.missingDirectives.map(missing => (
                  <div key={missing.directive} className="p-3 rounded-md bg-card border border-border">
                    <div className="flex items-start gap-3">
                      <code className="px-2 py-0.5 bg-warning/10 text-warning rounded text-xs font-mono shrink-0">
                      {missing.directive}
                      </code>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-foreground">{missing.importance}</p>
                        {missing.recommendation && (
                          <p className="text-sm text-muted-foreground mt-1">{missing.recommendation}</p>
                        )}
                        {missing.recommendedRule && (
                          <code className="block mt-2 px-2 py-1.5 bg-muted rounded text-xs font-mono">{missing.recommendedRule}</code>
                        )}
                        {missing.references && (
                          <a
          ...
          {attackSurface.length > 0 && (
            <div className="rounded-lg border border-border bg-card p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-semibold text-sm">Potential Attack Vectors</span>
              </div>
              <div className="space-y-3">
                  ...
                  <div key={atk.id} className="p-3 rounded-md bg-muted/50">
                    <div className="flex items-center gap-2 mb-2">
                      <span className={cn(
                        ...
                        atk.severity === "HIGH" ? "bg-destructive/20 text-destructive" : "bg-warning/20 text-warning"
                      )}>{atk.severity}</span>
                      <span className="text-sm text-muted-foreground">{atk.title}</span>
                    </div>
                    <p className="text-sm text-muted-foreground">{atk.impact}</p>
                    <div className="flex flex-wrap gap-2 mt-2">
                      {atk.causes.map((c, i) => (
                        <code key={i} className="px-1.5 py-0.5 bg-muted rounded text-xs">{c.directive}: {c.source}</code>
                      ))}
                    </div>
                  </div>
                ))}
      ...
      {!hasCSP && (
        <div className="rounded-lg border border-border bg-card p-6 text-center">
          ...
          <p className="text-sm text-muted-foreground">Selct a preset or enable directives below</p>
        </div>
      )}
      ...
      <div className="space-y-3 pt-4 border-t border-border">
        <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <span className="text-sm font-medium">
            Directives {enabledCount > 0 && <span className="text-primary">({enabledCount})</span>}
          </span>
        </div>
